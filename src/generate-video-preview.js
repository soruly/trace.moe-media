...
const generateVideoPreview = async (filePath, t, option = {mute: false}) => {
  if (t < 0) {
    return false;
  }
  ...
  const videoDuration = parseFloat(result[1]);
  if (t > videoDuration) {
    return false;
  }
  ...
  child_process.spawnSync(
    ...
    [
      ...
      "-preset",
      "faster",
      path.join(tempPath, "output.mp4")
    ],
    {encoding: "utf-8"}
  );
  const videoBuffer = fs.readFileSync(path.join(tempPath, "output.mp4"));
  fs.removeSync(tempPath);
  return {
